...
Slave::Slave(SSDB *ssdb, SSDB *meta, const char *ip, int port, bool is_mirror){
    thread_quit = false;
    this->ssdb = ssdb;
	this->_encoder = new ChessHashEncoder;
    this->meta = meta;
    ...
    case BinlogCommand::HSET:
		{
			...
			// TBD(kg): req[1] as value will include all existing values,
			// use typical kv set is more appropriate. will change to Merge
			std::string key;
			if(_encoder->decode_key(log.key(), &key) == -1){
				break;
			}
			log_trace("hset %s",
					  hexmem(key.data(), key.size()).c_str());
			if(ssdb->hset(key, req[1], log_type) == -1){
				return -1;
			}
		}
		break;
    case BinlogCommand::HDEL:
		{
			...
			log_trace("hset(instead of hdel) %s",
					  hexmem(key.data(), key.size()).c_str());
			if(ssdb->hset(key, req[1], log_type) == -1){
				return -1;
			}
